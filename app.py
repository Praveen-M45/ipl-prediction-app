...
def train_model(df):
    try:
        # Define features and target (removed 'season')
        features = ['team1', 'team2', 'venue', 'toss_winner']
        target = 'winner'
        ...
        X = df[['team1', 'team2', 'venue', 'is_toss_winner_team1']]
        y = df['winner']
...
# Predict winner
def predict_winner(model, le_team, le_venue, le_toss_winner, le_winner, team1, team2, venue, toss_winner):
    try:
        # Encode inputs
        ...
        # Prepare input data
        input_data = np.array([[team1_encoded, team2_encoded, venue_encoded, is_toss_winner_team1]])
...
def main():
    ...
    st.markdown("Select two teams, venue, and toss winner to predict the match winner.")
    ...
    if submit_button:
        ...
        # Make prediction
        winner, error = predict_winner(model, le_team, le_venue, le_toss_winner, le_winner, team1, team2, venue, toss_winner)
        if error:
            st.error(error)
        else:
